...
class VmCard(QFrame):
    """Карточка одной VM — главный элемент центральной колонки.

    В текущей бизнес-логике у пользователя ровно одна назначенная VM,
    поэтому это не QListWidget, а просто карточка.
    """

    def __init__(self) -> None:
        super().__init__()
        self.setObjectName("VmCard")
        self.setProperty("selected", True)  # одиночная VM = всегда «выбрана»

        layout = QHBoxLayout(self)
        layout.setContentsMargins(16, 14, 16, 14)
        layout.setSpacing(14)

        # Слева — индикатор-точка
        self.dot = QLabel()
        self.dot.setObjectName("VmDot")
        self.dot.setFixedSize(10, 10)
        layout.addWidget(self.dot, alignment=Qt.AlignmentFlag.AlignVCenter)

        # Центр — название и протокол
        ...
        # Справа — флаг/локация (стилистически как у Happ)
        self.region = QLabel("—")
    ...
    def set_vm(
        ...
        *,
        ...
        """Отобразить VM в карточке.

        Если `is_connected=True` → карточка в состоянии «подключён» (зелёная точка).
        Если `is_connected=False`, но vm_id+proxy переданы → состояние «отключён,
        но известна последняя VM» (серая точка, карточка сохраняется в списке).
        Если ни того, ни другого → пустое состояние «нет назначенной VM».
        """
        if vm_id and proxy:
        ...
        # пере-применить QSS после смены property
        self.style().unpolish(self)
        self.style().polish(self)
        # и для дочерней «точки» — её цвет зависит от родительского state
